...
class MonthlyBudgetModel {
  ...
  Future<void> load() async {
    _it = await Interpreter.fromAsset(
      'models/monthly_tf.tflite',
      options: InterpreterOptions()..threads = 2..useNnApiForAndroid = true,
    );
  }
  ...
  List<double> _ensure12(List<double> v) {
    ...
    if (data.length < lookback) {
      ...
      while (data.length < lookback) data.insert(0, last);
  ...
  Future<Map<String, dynamic>> buildPayload({
    ...
    final flex = catAvg.entries
        .where((e) => !fixedCats.contains(e.key))
        .toList()
    ...
    for (final kv in flex.take(3)) {
      ...
      if (cut > 0) {
        recs.add('Kurangi ${kv.key} sekitar ${_idr(cut)} '
                 'bulan depan (~${_idr(cut / 4.3)}/minggu).');
      }
    }
    if (recs.isEmpty) {
      recs.add('Pengeluaran stabil. Pertahankan pola saat ini dan cek kembali minggu depan.');
    }
    // return hasil menyesuaikan
    return {
      if (kpi3m != null) 'kpi': {'rmae3m': double.parse(kpi3m.toStringAsFixed(2))},
